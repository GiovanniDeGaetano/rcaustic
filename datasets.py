...
def get_dataset(config):
    ...
    if dataset_name.lower() == "rcaustic":
        train, test = _get_rcaustic(config)
...
def _get_rcaustic(config):
    ...
    data_path = data_root / "/projects/data/underwater/with_caustics-free_ground_truth/"
    assert data_path.is_dir(), f"Data path {data_path} does not exist."
       
    noise_images = list(data_path.rglob("*.JPG"))
     
    image_pairs = []
    for noise_img in noise_images:
        if (not any(suffix in noise_img.name for suffix in ['.cont', '.diff', '.bin']) and 'background' not in noise_img.name):
            base_name = '_'.join(noise_img.name.split('_')[:-1])
            img_name = f"{base_name}.back.JPG"  
            gt_img = noise_img.parent / "background" / img_name
            if gt_img.exists():
                image_pairs.append((noise_img, gt_img))
    
    split_idx = int(0.8 * len(image_pairs))
    train_pairs = image_pairs[:split_idx]
    test_pairs =  image_pairs[split_idx:]
    #test_pairs =  [f[0] for f in image_pairs[split_idx:]]	
    
    def _parse_images(file_paths):
        img_noise = tf.io.read_file(file_paths[0])
        img_noise = tf.image.decode_jpeg(img_noise, channels=3)
        img_noise = tf.image.resize(img_noise, image_size)
        img_noise = tf.cast(img_noise, tf.float32) / 255.0

        img_gt = tf.io.read_file(file_paths[1])
        img_gt = tf.image.decode_jpeg(img_gt, channels=3)
        img_gt = tf.image.resize(img_gt, image_size)
        img_gt = tf.cast(img_gt, tf.float32) / 255.0
        
        return img_noise, img_gt
    ...
    def _parse_image(file_path):
        ...
        img = img / 255.0
        return img
    train_files = [(str(f[0]), str(f[1])) for f in train_pairs]
    test_files = [(str(f[0]), str(f[1])) for f in test_pairs]
    #train_files = [str(f) for f in train_pairs]
    #test_files = [str(f) for f in test_pairs]    

  
    train_dataset = tf.data.Dataset.from_tensor_slices(train_files)
    train_dataset = train_dataset.map(_parse_images, num_parallel_calls=tf.data.AUTOTUNE)

    test_dataset = tf.data.Dataset.from_tensor_slices(test_files)
    test_dataset = test_dataset.map(_parse_images, num_parallel_calls=tf.data.AUTOTUNE)
    ...
    # Shuffle
    if shuffle:
        train_dataset = train_dataset.shuffle(len(train_files), seed=seed)
    ...
    print(f"Numero di immagini nel train dataset: {len(train_files)}")
    print(f"Numero di immagini nel test dataset: {len(test_files)}")
